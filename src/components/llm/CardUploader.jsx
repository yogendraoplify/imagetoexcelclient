...
const extractDataFromCard = async (card) => {
  ...
  if (card.frontImage) {
    ...
    images.push({
      ...
      source: {
        type: "base64",
        media_type: card.frontImage.type,
        data: base64,
      },
    });
  }
  ...
  if (card.backImage) {
    ...
    images.push({
      ...
      source: {
        type: "base64",
        media_type: card.backImage.type,
        data: base64,
      },
    });
  }
...
export default function CardUploader() {
  ...
  const [cardStatus, setCardStatus] = useState([]); // per-card status
  ...
  const submit = async () => {
    ...
    setLoading(true);
    setCardStatus(valid.map(() => "pending")); // pending | processing | done | error
    ...
    try {
      ...
      if (validRows.length === 0) {
        showToast("Failed to extract data from all cards", "error");
        return;
      }
      ...
      if (unique.length === 0) {
        showToast("All cards in this batch are duplicates", "error");
        return;
      }

      // Send to backend — only parsed JSON, no images
      ...
      if (total > 0) {
        showToast(
          `${savedCount} saved · ${batchDupes} duplicate(s) in batch · ${skippedCount} already in DB`,
          "info",
        );
      } else {
          ...
          <div>
            <div className="mb-7">
              ...
              <p className="text-gray-400 text-sm mt-1">
                Powered by Claude Vision — images are read directly, never
                stored.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
              {cards.map((card, i) => (
                <div key={card.id} className="relative">
                  ...
                  {cardStatus[i] && (
                    <div
                      className={`absolute -top-2 -right-2 z-10 flex items-center gap-1.5 px-2 py-1 rounded-full text-[11px] font-semibold border
                      ${
                        cardStatus[i] === "done"
                          ? "bg-[#F0FDF4] border-[#16A34A] text-[#16A34A]"
                          : cardStatus[i] === "processing"
                            ? "bg-[#EEF4FF] border-[#4674AB] text-[#4674AB]"
                            : cardStatus[i] === "error"
